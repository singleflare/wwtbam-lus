...
const DB_FILE = join(__dirname, 'data', 'mini-db.json');
...
app.post('/api/db/:collection', async (req, res) => {
  ...
  const existingIndex = db[collection].findIndex(item => item.id === id);

  if (existingIndex >= 0) {
    db[collection][existingIndex] = record;
  } else {
...
cNs.on('connection',s=>{
  ...
  s.on('revealCorrect',(chosenAns,correctAns,quesNum,prize,isMuted)=>{
    vNs.emit('revealCorrect',chosenAns,correctAns,quesNum,prize,isMuted)
    pNs.emit('revealCorrect',chosenAns,correctAns,quesNum,prize,isMuted)
    hNs.emit('revealCorrect',chosenAns,correctAns,quesNum,prize,isMuted)
  })
  ...
  s.on('updatePrizeInfo',(prizeInfo,quesNum)=>{
    ...
    pNs.emit('updatePrizeInfo',prizeInfo,quesNum)
  })
  s.on('playerInfo',msg=>{
